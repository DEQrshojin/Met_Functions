...
# 5 September 2018
# Returns a dataframe of direct beam solar radiation and photosynthically active radiation (PAR)
# based on clear-sky solar radiation. Requires the input of: start/end time, time step,
# location (lat/lon), timezone and atmospheric conditions (only constant pressure at the moment)
...
solar2par <- function(strDate = NULL, endDate = NULL, timeStep = NULL, lat = NULL, lon = NULL,
                      tz = NULL,  pres = NULL) {
  ...
  x$hr = hour(x$ts) + minute(x$ts) / 60
  
  x$doy = as.numeric(strftime(x$ts, format = "%j"))
  ...
  x$solar <- ifelse(x$mAir > 0,
                    ...
                    0) # Calculation of direct beam solar Units in W/m2
  ...
  # Conversion factor from W/m2 to umol/m2/s = 4.57; only 45% tho in the 400-700 nm wv-len range
